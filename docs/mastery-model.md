...
- `local_course_agent.learning.mastery.builders`：创建 state、knowledge point、mastery record、mistake record。
- `local_course_agent.learning.mastery.normalization`：规范化 legacy state，并处理文本、引用和 ID 的稳定化。
- `local_course_agent.learning.mastery.schema`：兼容导出 builders 与 normalization 中的旧 schema 入口。
- `local_course_agent.learning.mastery.policy`：定义分数边界、难度权重、分数增减、等级划分和复习间隔建议。
- `local_course_agent.learning.mastery.operations`：串联 normalization、builders 与 policy 完成知识点 upsert、答题更新和错题订正。
- `local_course_agent.learning.mastery`：兼容入口，只导出旧调用方依赖的公开函数。
...
核心入口是 `apply_answer_result()`。实现位于 `learning.mastery.operations`，兼容入口 `learning.mastery` 继续导出它；内部先用 `mastery.normalization.normalize_state()` 固定输入形态，再用 `mastery.policy.score_delta()` 和 `mastery.policy.review_suggestion()` 计算策略结果：
