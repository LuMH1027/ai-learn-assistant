...
4. 未配置模型时使用明确标记的本地回退回答；配置模型后的瞬时错误会重试，持续失败则返回请求错误，不把连接故障伪装成正常模型结果。

## 按需联网与来源融合

联网不是每轮固定执行。最多三轮的 ReAct planner 根据问题、当前对话历史、附件、学习模式和已有 observation 选择：

- `final`：无需工具，结束 ReAct 阶段并进入最终 LLM responder；
- `course_search`：检索当前课程；
- `web_search`：联网搜索；
- `course_and_web_search`：同时使用两类证据；
- `clarify`：问题不完整时请求补充信息。

所有正常问题都会先经过 planner；简单问候或自我介绍问题也由 planner 返回 `final`，不会在 planner 前被硬编码短路。planner 会把工具结果写入 observation，同类工具已执行过时不会重复调用。如果计划中的课程或网页工具已经命中可用证据，流程会直接进入最终 responder，省去二次 planner 判断。搜索只发送搜索查询，不发送课程原文或附件内容；带附件的请求禁止联网。网页工具返回内容按不可信数据处理：只接受 `http/https` URL 的结果，忽略其中试图改变系统规则或要求执行操作的指令。

融合回答使用两套编号：`[L1]` 表示本地课程来源，`[W1]` 表示网页来源。网页来源在界面中显示为可点击外链；课程来源继续打开本地预览。MCP 调用状态会记录在 Agent 处理过程里。

聊天接口通过 SSE/NDJSON 发送 `status`、`thought`、`delta`、`error` 和 `done` 事件，并通过 HTTP/1.1 chunked transfer 立即刷新。前端逐步显示 ReAct 思考和状态；当 planner 返回 `final` 后，会继续显示“正在调用最终模型生成回答…”，直到 responder 返回首个 token。前端异步消费每个 delta 并实时渲染安全 Markdown；模型不支持 streaming 时保留阶段反馈并使用普通生成。瞬时 LLM 请求错误最多共尝试五次，重试前发送 `llm_retry` 状态；已经输出 token 后不会重放请求，避免重复回答。
