...
| 文档解析模块 | `local_course_agent/parser.py`、`mineru_api.py` | PDF/文本/图片解析，优先 MinerU，失败降级 |
| RAG 检索模块 | `local_course_agent/rag.py` | 文本切片、关键词评分、引用来源、摘要和练习题 |
| 大模型模块 | `local_course_agent/llm.py` | 调用 Kimi OpenAI-compatible 接口，支持文本和图片内容块 |
| 文件型记忆模块 | `local_course_agent/store.py` | 保存聊天记录、课程记忆、笔记 |
| 上传模块 | `local_course_agent/uploads.py` | 保存课程资料上传和聊天临时附件 |
| HTTP 服务模块 | `local_course_agent/server.py` | 提供 API，协调扫描、解析、检索、生成和预览 |
| 前端模块 | `web/index.html`、`web/app.js`、`web/styles.css` | 三栏界面、文件树、预览、聊天、笔记抽屉 |
