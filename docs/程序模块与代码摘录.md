...
| 文件型记忆模块 | `local_course_agent/store/` | 保存聊天记录、课程记忆、笔记和掌握度 |
| 上传模块 | `local_course_agent/uploads.py` | 保存课程资料上传和聊天临时附件 |
| HTTP 服务模块 | `local_course_agent/server.py` | 提供 API，协调扫描、解析、检索、生成和预览 |
| 前端模块 | `frontend/src/*.vue`、`frontend/src/stores/*`、`frontend/src/services/api.ts` | Vue 三栏工作台、文件树、预览、聊天、笔记、掌握度和 API service layer |
