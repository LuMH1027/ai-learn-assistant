...
flowchart LR
    ...
    API --> Store["文件型状态存储<br/>store/"]
    API --> Ops["运行状态诊断<br/>ops/config_status/"]

    Learning --> RAG["课程 RAG<br/>retrieval/rag/"]
    Chat --> RAG
    ...
    Artifacts --> Summary["Map-Reduce 摘要<br/>learning/summary/"]
    Summary --> LLM
    RAG --> Vector["持久向量索引<br/>retrieval/vector_index.py"]

    Scanner --> CourseFiles["本地课程资料目录"]
    Parser --> CourseFiles
    Store --> DataFiles["data/course_memory/*<br/>messages/notes/memory"]
...
- 课程资料来自本地文件夹，一级文件夹识别为课程；每门课程拥有独立文件树、知识库索引、聊天记录、记忆和笔记。
...
| GET | `/api/courses/{course_id}/dashboard` | 获取课程资料、活动和掌握度概览 |
...
| 模块 | 文件 | 职责 |
| --- | --- | --- |
| 前端工作台 | `frontend/src/*.vue`、`frontend/src/stores/*` | 三栏课程工作台、文件树、预览、聊天、笔记和状态面板 |
...
| 课程 API 编排 | `local_course_agent/api/course/` | 索引、上传、摘要、练习题、笔记、dashboard 和 mastery API 适配 |
...
| 学习服务门面 | `local_course_agent/learning/service.py` | 保持旧 import 兼容，薄包装索引、学习产物和文件工具 |
...
| 掌握度模型 | `local_course_agent/learning/mastery/` | 知识点、掌握分、错题和复习间隔的纯数据模型 |
| Map-Reduce 摘要 | `local_course_agent/learning/summary/` | evidence 规范化、章节分组、map/reduce prompt 构造和 LLM 摘要执行 |
| RAG 编排 | `local_course_agent/retrieval/rag/` | 知识库持久化、检索编排、引用生成、摘要/练习题本地 fallback |
...
| RAG 评测 | `local_course_agent/retrieval/evaluation/`、`scripts/rag_eval.py` | 基于用例评估引用命中、答案术语覆盖、禁用词和引用支撑 |
...
| 文件型状态 | `local_course_agent/store/` | 用 JSON/Markdown 保存聊天、记忆、笔记和掌握度 |
| 上传管理 | `local_course_agent/uploads.py` | 保存课程资料上传和聊天临时附件，限制路径和总大小 |
| 运行诊断 | `local_course_agent/ops/config_status/`、`ops/telemetry/` | 汇总配置健康状态；提供请求级/任务级 telemetry recorder 供索引、检索和 LLM 诊断使用 |
